...
Uses `db_conn` (per-test rollback isolation, see `tests/conftest.py`). Retire
via raw SQL `UPDATE ... SET deleted_at = now()` -- no `soft_delete`/
`soft_delete_variant` port method exists yet (that's PR2's job); this PR
only proves the views themselves filter correctly, which the repository's
own reads (`tests/integration/db/test_product_repository.py`) already rely
on for the `ON`-vs-`WHERE` trap.
