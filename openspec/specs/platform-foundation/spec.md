...
The repository MUST include `supabase init` wiring with no schema SQL present.

#### Scenario: Config exists, migrations empty
- GIVEN the `supabase/` directory after `supabase init`
- WHEN `supabase/config.toml` and `supabase/migrations/` are inspected
- THEN `config.toml` MUST exist AND `migrations/` MUST contain no schema SQL files
...
The Serwist PWA configuration MUST document a concrete runtime-caching strategy for public catalog routes; actual cache population is out of scope for this change.

#### Scenario: Strategy documented, not yet populated
- GIVEN the Serwist configuration files
- WHEN the caching strategy for public catalog routes/images is inspected
- THEN a named strategy (e.g. StaleWhileRevalidate) MUST be documented in configuration or design docs
- AND no real catalog route or populated cache entry is required to exist yet
