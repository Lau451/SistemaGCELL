...
**Rationale**: `initial-scaffolding/design.md:34` pins "**Admin URL contract**: the `(admin)` route group serves under `/admin/*`". A route group adds **no** URL segment, so the proposal's `app/(admin)/login/page.tsx` would serve `/login` — which does **not** match `isAdminOrMutatingRequest` in `runtime-caching.ts:42` and would fall through to Serwist's `defaultCache`, caching a login page in a shared cache. `/admin/login` (and `/admin`, `/admin/products`) is `NetworkOnly` by matcher rule #1 with **zero changes** to `runtime-caching.ts` (verified: `url.pathname.startsWith("/admin/")`).

**Correction — the API proxy route is a separate case, NOT covered by this same "zero changes" claim.** `frontend/src/app/api/admin/products/route.ts` serves `/api/admin/products`, a genuinely different prefix from `/admin/*` — `isAdminOrMutatingRequest` did NOT match it (caught by the PR2 conformance test's own RED failure, not assumed). This path must stay OUTSIDE `/admin/*` on purpose: `proxy.ts`'s `config.matcher: ["/admin/:path*"]` deliberately excludes it so the JSON route can return its own `401` instead of an HTML redirect-to-login — so the fix is NOT to rename the route under `/admin/*` (that would silently break the exclusion and send unauthenticated JSON callers an HTML redirect instead of `401`). The correct, minimal fix is a small, deliberate extension to `runtime-caching.ts`: add `ADMIN_API_PREFIX = "/api/admin"` and an `url.pathname.startsWith(ADMIN_API_PREFIX)` check to `isAdminOrMutatingRequest`, mirroring the file's own existing `CATALOG_API_PREFIX`/`isCatalogApiRead` pattern. This is the ONE deliberate edit this change makes to that otherwise-pinned file; the conformance test's pinned SHA256 is updated to match, and any FURTHER edit still fails it.
...
| Conformance (FE) | SW matcher | Extend the existing `lib/pwa/__tests__/catalog-route-conformance.test.ts` style: `/admin`, `/admin/login`, `/admin/products`, `/api/admin/products` all resolve to the `NetworkOnly` entry; assert `runtime-caching.ts` matches the pinned hash INCLUDING the one deliberate `/api/admin` prefix extension (see the corrected "Decision: admin routes live at..." section above) — not byte-identical to the pre-`admin-panel-auth` state. |
