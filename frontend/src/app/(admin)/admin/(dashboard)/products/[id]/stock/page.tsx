/**
 * `/admin/products/[id]/stock` — stock management page. Server Component
 * fetching `app/api/admin/products/{id}` (design.md's Data Flow: "fetch
 ...
 *
 * Split out from the combined `/admin/products/[id]` edit page: this
 * route is stock-only (record-movement form, movement history, variant
 * switcher) so an admin arriving from "Stock" never sees the product-info
 * form/image manager, and the info-edit page never shows stock controls.
 * `fetchAdminProduct` is still needed here for `product.variants` (fed to
 * `StockManager` and `VariantSwitcher`) and `product.name`/`product.model`
 * (shown as page context).
 *
 ...
 * renders `StockManager` — the only surface for
 ...
 *
 * A product with ZERO variants must still render this page (not 404) —
 * `hasVariants` guards `resolveActiveVariant` being called at all
...
import type { ProductFormVariant } from "../../product-form";
...
export default async function ProductStockPage({
  ...
  // (admin-product-management: "A Product May Have Zero Active Variants
  // Without Being Retired" — this page MUST stay reachable) — DD4's
